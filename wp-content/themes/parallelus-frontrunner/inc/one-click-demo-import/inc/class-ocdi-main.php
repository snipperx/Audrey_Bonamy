...
require PT_OCDI_PATH . 'inc/class-ocdi-redux-importer.php';
...
class PT_One_Click_Demo_Import {
	...
	<div class="ocdi  wrap  about-wrap">
		...
		<h1 class="ocdi__title upthemes_starter-kit-title dashicons-before  dashicons-migrate">
			<?php esc_html_e( 'Starter Kits', 'upthemes' ) ?>
			<span class="upthemes_starter-kit-subtitle"><?php esc_html_e( 'One Click Setup', 'upthemes' ) ?></span>
		</h1>
		...
		echo wp_kses_post( apply_filters( 'pt-ocdi/plugin_notices', '' ) );
		?>

<!--		<div class="ocdi__intro-notice notice notice-warning is-dismissible">-->
<!--			<p>--><?php //printf( esc_html__( 'Before you begin, make sure all required plugins are %sinstalled and activated%s.', 'pt-ocdi' ), '<a href="themes.php?page=theme-plugins">', '</a>' ); ?><!--</p>-->
<!--		</div>-->

		<div class="ocdi__response js-ocdi-ajax-response fadein"></div>
		...
		<?php elseif ( 1 < count( $this->import_files ) ) : ?>
			...
			<div class="ocdi__multi-select-import">
				...
				<p class="ocdi__button-container">
					<button class="ocdi__button  button  button-hero  button-primary  js-ocdi-import-data"><?php esc_html_e( 'Apply Starter Kit', 'pt-ocdi' ); ?></button>
				</p>
				...
				<h2 class="ocdi__select-title"><?php esc_html_e( 'Select a kit to apply:', 'pt-ocdi' ); ?></h2>
	...
	public function import_demo_data_ajax_callback() {
		...
		/**
		 * 6. Import Redux options
		 */
		if ( ! empty( $this->selected_import_files['redux'] ) && empty( $this->frontend_error_messages ) ) {
			$this->import_redux( $this->selected_import_files['redux'] );
		}
		...
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sSetup is complete and you can check your site to ensure everything imported correctly.%5$s%6$sBe sure to %7$sactivate your license%8$s for automatic updates.%9$s', 'pt-ocdi' ),
				...
				'</p>',
				'<p>',
				'<a href="index.php?page=upthemes_sl_license">',
				'</a>',
				'</p></div>'
			);
		}
	...
	private function import_redux( $redux_import_files ) {
		// Try to import the redux settings.
		$results = OCDI_Redux_Importer::import( $redux_import_files );

		// Check for errors.
		if ( is_wp_error( $results ) ) {
			$this->frontend_error_messages .= '<br/>' . $results->get_error_message();
		}
	}
