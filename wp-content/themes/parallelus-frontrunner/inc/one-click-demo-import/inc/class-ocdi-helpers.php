	...
	public static function download_import_files( $import_file_info, $start_date = '' ) {
		...
		if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
			...
			foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
				...
				$redux_filename = apply_filters( 'pt-ocdi/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . time() . apply_filters( 'pt-ocdi/downloaded_redux_file_suffix_and_file_extension', '.json' );

				// Download the Redux import file.
				$file_path = self::get_content_from_url( $redux_item['file_url'], $redux_filename );
