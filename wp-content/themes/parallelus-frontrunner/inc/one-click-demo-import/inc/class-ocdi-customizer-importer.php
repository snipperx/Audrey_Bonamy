...
class OCDI_Customizer_Importer {
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( apply_filters( 'pt-ocdi/customizer_import_images', true ) ) {
			...
			$data['options'] = self::import_customizer_options_images( $data['options'] );
		}
	...
	/**
	 * Helper function: Customizer import - imports images for settings saved as options.
	 *
	 * @param array $options An array of customizer options.
	 * @return array The options array with any new import data.
	 */
	private static function import_customizer_options_images( $options ) {
		foreach ( $options as $key => $val ) {
			if ( is_array( $val ) ) {
				$options[ $key ] = self::import_customizer_options_images( $val );
			} else if ( self::customizer_is_image_url( $val ) ) {
				$data = self::customizer_sideload_image( $val );
				if ( ! is_wp_error( $data ) ) {
					$options[ $key ] = $data->url;

					// Handle header image controls.
					if ( isset( $options[ $key . '_data' ] ) ) {
						$options[ $key . '_data' ] = $data;
						update_post_meta( $data->attachment_id, '_wp_attachment_is_custom_header', get_stylesheet() );
					}
				}
			}
